...
def _instructions_systeme(conversation: ConversationWhatsApp) -> str:
    return (
        ...
        "- Le contexte ci-dessous (formules, programmes, FAQ) est complet et à jour : "
        "utilise-le pour répondre avec assurance à toute question sur les tarifs, "
        "programmes, horaires, localisation ou politiques de la salle, même si la "
        "réponse demande de combiner plusieurs informations fournies (ex: comparer deux "
        "formules). N'escalade PAS juste par prudence si la réponse est dans le contexte.\n"
        "- Aucun conseil médical, nutritionnel ou d'entraînement personnalisé.\n"
        "- Escalade (needs_human=true) UNIQUEMENT si : la question porte sur le compte "
        "personnel du client au-delà de ce qui est fourni ci-dessous, nécessite une "
        "action humaine (paiement, réclamation, inscription, modification de compte), "
        "ou si l'information demandée n'existe vraiment pas dans le contexte fourni.\n\n"
